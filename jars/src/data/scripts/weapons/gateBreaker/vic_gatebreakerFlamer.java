...
public class vic_gatebreakerFlamer extends BaseCombatLayeredRenderingPlugin implements OnFireEffectPlugin,
        OnHitEffectPlugin,
        EveryFrameWeaponEffectPlugin
{
    private static final float SOFT_FLUX_FRACTION = 0f;
    ...
    protected List<vic_gatebreakerFlamer> trailsOne;
    protected List<vic_gatebreakerFlamer> trailsTwo;


    public vic_gatebreakerFlamer()
    {
    }
    ...
    public void advance(float amount, CombatEngineAPI engine, WeaponAPI weapon)
    {
        ...
        if (!runOnce)
        {
            ...
            if (!ship.hasListenerOfClass(vic_gatebreakerFlamerDamageMod.class))
            {
                ship.addListener(new vic_gatebreakerFlamerDamageMod());
            }
        }

        if (trailsOne == null) return;
            Iterator<vic_gatebreakerFlamer> iter = trailsOne.iterator();
            while (iter.hasNext()) {
        ...
        if (trailsTwo != null)
        {
            Iterator<vic_gatebreakerFlamer> iterTwo = trailsTwo.iterator();
        ...
        if (weapon.getShip() != null)
        {
            ...
            for (vic_gatebreakerFlamer curr : trailsOne)
            {
                if (curr.proj != null)
                {
            ...
            if (trailsTwo != null)
            {
                for (vic_gatebreakerFlamer curr : trailsTwo)
                {
                    if (curr.proj != null)
                    {
                        Vector2f.add(com, curr.proj.getLocation(), com);
            ...
            if (weight > 0.1f)
            {
                ...
                Global.getSoundPlayer().playLoop("vic_gatebreaker_flamer_loop", ship, 1f, volume, com, ship.getVelocity());
            }
        }
        ...
        {
            for (vic_gatebreakerFlamer trail : trailsOne)
            {
            ...
            if (trailsTwo != null)
            {
                for (vic_gatebreakerFlamer trail : trailsTwo)
                {
                    //trail.proj.setFacing(trail.proj.getFacing() + 180f * amount);
                    if (trail.prev != null && !trail.prev.isExpired() && Global.getCombatEngine().isEntityInPlay(trail.prev))
                    {
...
//								   rampUp, dur, inverted);
    ...
    public void onFire(DamagingProjectileAPI projectile, WeaponAPI weapon, CombatEngineAPI engine)
    {
        ...
        {
            String prevKey = "vic_gatebreaker_flamer_prev" + weapon.getShip().getId() + "_" + weapon.getSlot().getId();
            DamagingProjectileAPI prev = (DamagingProjectileAPI) engine.getCustomData().get(prevKey);

            vic_gatebreakerFlamer trail = new vic_gatebreakerFlamer(projectile, prev);
            ...
            if (trailsOne == null)
            {
                trailsOne = new ArrayList<vic_gatebreakerFlamer>();
            }
        ...
        } else
        {
            ...
            {
                barrel++;
                String prevKey = "vic_gatebreaker_flamer_prev1_" + weapon.getShip().getId() + "_" + weapon.getSlot().getId();
                DamagingProjectileAPI prev = (DamagingProjectileAPI) engine.getCustomData().get(prevKey);
            ...
            {
                barrel = 0;
                String prevKey = "vic_gatebreaker_flamer_prev2_" + weapon.getShip().getId() + "_" + weapon.getSlot().getId();
                DamagingProjectileAPI prev = (DamagingProjectileAPI) engine.getCustomData().get(prevKey);
                ...
                if (trailsTwo == null)
                {
                    trailsTwo = new ArrayList<vic_gatebreakerFlamer>();
                }
...
//			offset.y += vel.y * 1f;
    ...
    public vic_gatebreakerFlamer(DamagingProjectileAPI proj, DamagingProjectileAPI prev)
    {
        this.proj = proj;
        this.prev = prev;
    ...
    public static class vic_gatebreakerFlamerDamageMod implements DamageDealtModifier
    {
        public vic_gatebreakerFlamerDamageMod()
        {
        ...
        public String modifyDamageDealt(Object param, CombatEntityAPI target, DamageAPI damage, Vector2f point, boolean shieldHit)
        {
            ...
            if (param instanceof DamagingProjectileAPI && shieldHit && target instanceof ShipAPI)
            {
                ...
                if (weapon == null)
                    return null;
                if (weapon.getId().contains("vic_gatebreaker_flamer"))
                {
                    //if (shieldHit)
                    //{
                    damage.getModifier().modifyMult("vic_gatebreaker_flamer", 1f - SOFT_FLUX_FRACTION);
                    ...
                    return "vic_gatebreaker_flamer";
                    /*} else {
                        if (Misc.random.nextFloat() <= 0.1f)
                        {
                            damage.getModifier().modifyMult("vic_gatebreaker_flamer", 3f);
                            return  "vic_gatebreaker_flamer";
                        }
                    }*/
                }
