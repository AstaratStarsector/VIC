...
public class vic_azhdayaHoming extends BaseEveryFrameCombatPlugin {
	 ...
	 */
	public vic_azhdayaHoming(@NotNull DamagingProjectileAPI proj, CombatEntityAPI target) {
		this.proj = proj;
		this.target = target;
	...
	private void applySwarmOffset() {
		...
		while (i > 0 && target != null) {
			...
			Vector2f potPoint = MathUtils.getRandomPointInCircle(target.getLocation(), target.getCollisionRadius() * 0.5f);
