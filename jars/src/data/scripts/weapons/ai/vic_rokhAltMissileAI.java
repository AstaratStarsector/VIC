...
public class vic_rokhAltMissileAI extends VIC_BaseMissile {
    ...
    final String STAGE_TWO_WEAPON_ID = "vic_raum_rokh_alt_sub";
    ...
    public vic_rokhAltMissileAI(MissileAPI missile, ShipAPI launchingShip) {
...
*/
    ...
    float minesTotal = popOuts.size() * 8;
    int currentCycle = 0;
    float volleyTime = 3f;
    ...
    public void advance(float amount) {
        ...
        */
        ...
        if ((targetPoint != null || acquireTarget()) && !mirvNow) {
            ...
            if (MathUtils.isWithinRange(missile.getLocation(), targetLoc, MIRV_DISTANCE + target.getCollisionRadius()) ||
                    missile.getFlightTime() > missile.getMaxFlightTime() - volleyTime) {
                mirvNow = true;
            }
            /*
        ...
        if (mirvNow) {
            ...
            mirvTimer += amount;
            float minesToLaunch = MathUtils.clamp(mirvTimer / volleyTime * minesTotal, 0, minesTotal);
            while (minesToLaunch - minesLaunched >= 1) {
                ...
                for (Vector2f loc : popOuts) {
                    ...
                    Global.getSoundPlayer().playSound("vic_afreet_large_stream", 1, 0.25f, pos, missile.getVelocity());
                     ...
                     */
                    ((MissileAPI) mine).setMissileAI(new VIC_SwarmMirvAI(((MissileAPI) mine), launchingShip));
                    ((Missile.GuidedMissileAIWrapper) ((MissileAPI) mine).getMissileAI()).setTarget(target);
                    speed.scale(0.5f);
                ...
                minesLaunched++;
                if (currentCycle >= popOuts.size() - 1) {
                    currentCycle = 0;
                }
            }

            DamagingExplosionSpec explosion = new DamagingExplosionSpec(0.1f,
                    200,
                    100,
            ...
            explosion.setDamageType(DamageType.KINETIC);
