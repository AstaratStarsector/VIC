...
public class II_ImpulseBoosterAI implements ShipSystemAIScript {
    ...
    public void advance(float amount, Vector2f missileDangerDir, Vector2f collisionDangerDir, ShipAPI target) {
        ...
        if (tracker.intervalElapsed()) {
            ...
            float boostScale = II_ImpulseBoosterStats.BASELINE_MULT.get(ship.getHullSize());
            if (ship.getEngineController().isAccelerating()) {
                direction.y += II_ImpulseBoosterStats.BASELINE_MULT.get(ship.getHullSize()) - II_ImpulseBoosterStats.FORWARD_PENALTY.get(ship.getHullSize());
                boostScale -= II_ImpulseBoosterStats.FORWARD_PENALTY.get(ship.getHullSize());
            } else if (ship.getEngineController().isAcceleratingBackwards() || ship.getEngineController().isDecelerating()) {
                direction.y -= II_ImpulseBoosterStats.BASELINE_MULT.get(ship.getHullSize()) - II_ImpulseBoosterStats.REVERSE_PENALTY.get(ship.getHullSize());
                boostScale -= II_ImpulseBoosterStats.REVERSE_PENALTY.get(ship.getHullSize());
            }
            if (ship.getEngineController().isStrafingLeft()) {
                direction.x -= 1f;
                boostScale += 1f - II_ImpulseBoosterStats.BASELINE_MULT.get(ship.getHullSize());
