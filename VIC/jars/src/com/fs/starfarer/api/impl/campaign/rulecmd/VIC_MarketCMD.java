...
public class VIC_MarketCMD extends BaseCommandPlugin {
	...
	protected void VBombMenu(){
		...
		if (market.getHazardValue() != 1){
			info.addPara("Bombardment difficulty rating: %s", opad, h, "" + Math.round(defenderStr * market.getHazardValue()));
			info.addStatModGrid(width, 50, opad, small, hazardMult, true, statPrinter(true));
		}
		...
		boolean canBombard = organics >= temp.bombardCost;

		LabelAPI label = text.addPara("A bombardment requires %s organics. " +
						"You have %s organics.",
				h, "" + temp.bombardCost, "" + organics);
		label.setHighlight("" + temp.bombardCost, "" + organics);
	...
	protected void VBombResults(){
		...
		playerFleet.getCargo().removeCommodity(Commodities.ORGANICS ,getBombardmentCost(market, playerFleet));
		AddRemoveCommodity.addCommodityLossText(Commodities.ORGANICS, getBombardmentCost(market, playerFleet), text);
		...
		int stabilityPenalty = 3;

		RecentUnrest.get(market).add(stabilityPenalty, "Resent bombardment");
	...
	public static int getBombardmentCost(MarketAPI market, CampaignFleetAPI fleet) {
		...
		return Math.round(result * hazardMult);
	}
